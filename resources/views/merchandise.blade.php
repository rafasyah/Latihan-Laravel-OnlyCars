...
@section('title', 'Add To Merchandise')
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
    @foreach ($merchandise as $m)
        <div  data-aos="fade-up"class="bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
            <a href="#">
                <img class="w-full h-48 object-cover rounded-t-lg" src="{{ asset('storage/' . $m->image_merch) }}" alt="{{ $m->name }}" />
            </a>
            <div class="p-5">
                <a href="#">
                ...
                <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">{{ $m->price }}</p>
                <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">{{ $m->description }}</p>
             

                <a href="{{ route('merchandise.show', $m->id) }}" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white 
